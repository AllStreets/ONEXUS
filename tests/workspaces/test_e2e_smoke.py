...
Exercises the full lifecycle without mocking internals:
  ...
  3. Create a GrantsStore and grant a capability.
  ...
  8. Destroy the workspace (grants store is on disk; runtime stops agents).
"""
...
from nexus.workspaces.grants import GrantsStore
...
def test_full_workspace_lifecycle(tmp_path, mgr, workspaces_root):
    ...
    # ── 3. GrantsStore ───────────────────────────────────────────────────
    grants_db = ws_dir / "grants.sqlite"
    gs = GrantsStore(grants_db)
    gs.init_db()
    gs.grant("aider", "fs.write.workspace", scope=str(ws_dir))
    assert gs.has("aider", "fs.write.workspace", scope=str(ws_dir))
