...
function trustClass(score) {
  if (score >= 100) return 'trust-purple';
  if (score >= 75)  return 'trust-green';
  if (score >= 50)  return 'trust-cyan';
  if (score >= 25)  return 'trust-amber';
  return 'trust-red';
}

function glowClass(score) {
  if (score >= 100) return 'glow-perfect';
  if (score >= 75)  return 'glow-max';
  if (score >= 50)  return 'glow-high';
  if (score >= 25)  return 'glow-med';
  return 'glow-low';
}
...
function renderEmpty(iconName, msg) {
  return `<div class="panel-empty">${icon(iconName, 32)}<span>${msg}</span></div>`;
}
...
const ModuleList = {
  ...
  render() {
    ...
    this.bodyEl.innerHTML = list.map(m => {
      ...
      const cat = this.guessCategory(m);
      return `<div class="module-item fade-enter${sel}" data-name="${m.name}">
        ...
        <span class="module-trust-badge ${trustClass(m.trust)}">${m.trust}</span>
      </div>`;
    }).join('');
...
const TrustGauges = {
  ...
  buildGaugeSVG(score, color) {
    ...
    // Arc goes from 135deg to 405deg (270 degree sweep)
    ...
    return `<svg class="gauge-svg" viewBox="0 0 90 90">
      <circle class="gauge-bg" cx="45" cy="45" r="${r}"
        stroke-dasharray="${totalArc} ${circumference}"
        stroke-dashoffset="0"
        transform="rotate(${startAngle} 45 45)"
        stroke-linecap="round" />
      <circle class="gauge-fill" cx="45" cy="45" r="${r}"
        stroke="${color}"
        stroke-dasharray="${totalArc} ${circumference}"
        stroke-dashoffset="${dashOffset}"
        transform="rotate(${startAngle} 45 45)" />
      <text class="gauge-value" x="45" y="47">${score}</text>
    </svg>`;
  },
  ...
  render() {
    ...
    // Click gauge to select module in list
    ...
    this.trackEl.innerHTML = `<div class="timeline-line"></div>${sorted.map(e => {
      ...
      return `<div class="timeline-node">
        <div class="timeline-tooltip">
          ...
          ${payloadStr ? `<div style="color:var(--text-dim);font-size:10px;margin-top:2px;font-family:var(--mono)">${payloadStr}</div>` : ''}
